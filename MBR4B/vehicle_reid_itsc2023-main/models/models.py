...
#     def __init__(self, channels, reduction=16, heads=4, height=16, width=16):
...
#         self.scale = (channels // reduction // heads) ** -0.5
#         self.rel_h = nn.Parameter(torch.randn([1, heads, channels // reduction // heads, 1, height]), requires_grad=True)
#         self.rel_w = nn.Parameter(torch.randn([1, heads, channels // reduction // heads, width, 1]), requires_grad=True)
#
#     def forward(self, x):
#         batch, channels, height, width = x.size()
#         q = self.query_conv(x).view(batch, self.heads, channels // self.heads // self.reduction, -1)
#         k = self.key_conv(x).view(batch, self.heads, channels // self.heads // self.reduction, -1)
#         v = self.value_conv(x).view(batch, self.heads, channels // self.heads, -1)
#
#         q = q * self.scale
#         content_content = torch.matmul(q.permute(0, 1, 3, 2), k)
#
#         rel_h_view = self.rel_h.repeat(1, 1, 1, width, 1).view(1, self.heads, channels // self.reduction // self.heads, -1)
#         rel_w_view = self.rel_w.repeat(1, 1, 1, 1, height).view(1, self.heads, channels // self.reduction // self.heads, -1)
#         content_position = torch.matmul((rel_h_view + rel_w_view).permute(0, 1, 3, 2), q)
#
#         attention = self.softmax(content_content + content_position)
#         out = torch.matmul(attention, v.permute(0, 1, 3, 2))
#         out = out.permute(0, 1, 3, 2).contiguous().view(batch, -1, height, width)
#         out = self.fc(out)
#         return x + out


class HydraAttention(nn.Module):#83.03
    def __init__(self, channels, reduction=16, heads=4, height=16, width=16):
        super(HydraAttention, self).__init__()
        self.heads = heads
        self.reduction = reduction
    ...
    def forward(self, x):
        ...
        rel_h_view = self.rel_h.repeat(1, 1, 1, width, 1).view(1, self.heads, channels // self.heads // self.reduction, -1)
        rel_w_view = self.rel_w.repeat(1, 1, 1, 1, height).view(1, self.heads, channels // self.heads // self.reduction, -1)
        content_position = torch.matmul((rel_h_view + rel_w_view).permute(0, 1, 3, 2), q)
...
class Bottleneck_Transformer(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, heads=4, resolution=None, use_mlp = False):
        ...
        self.conv2.append(HydraAttention(planes, reduction=16, heads=heads, height=int(resolution[0]), width=int(resolution[1])))
        if stride == 2:
...
class Conv_HydraAttention_2G(nn.Module):
    def __init__(self, c_in, c_out, resolution=[16, 16], heads=4, reduction=16) -> None:
        super().__init__()
        self.conv2 = nn.Conv2d(c_in//2, c_out//2, kernel_size=3, stride=1, padding=1, bias=False)
        self.hydra_attention_1 = HydraAttention(c_out//2, reduction=reduction, heads=heads, height=int(resolution[0]), width=int(resolution[1]))

    def forward(self, x):
        x_1 = self.conv2(x[:, :x.size(1)//2, :, :])
        x_2 = self.hydra_attention_1(x[:, x.size(1)//2:, :, :])
        x = torch.cat((x_1, x_2), dim=1)
        return x


class Conv_HydraAttention_4G(nn.Module):
    def __init__(self, c_in, c_out, resolution=[16, 16], heads=4, reduction=16) -> None:
        super().__init__()
        self.conv2 = nn.Conv2d(c_in//2, c_out//2, kernel_size=3, stride=1, padding=1, groups=2, bias=False)
        self.hydra_attention_1 = HydraAttention(c_out//4, reduction=reduction, heads=heads, height=int(resolution[0]), width=int(resolution[1]))
        self.hydra_attention_2 = HydraAttention(c_out//4, reduction=reduction, heads=heads, height=int(resolution[0]), width=int(resolution[1]))

    def forward(self, x):
        x_12 = self.conv2(x[:, :x.size(1)//2, :, :])
        x_3 = self.hydra_attention_1(x[:, x.size(1)//2:x.size(1)//2 + x.size(1)//4, :, :])
        x_4 = self.hydra_attention_2(x[:, x.size(1)//2 + x.size(1)//4:, :, :])
        x = torch.cat((x_12, x_3, x_4), dim=1)
        return x


class HydraAttention_2G(nn.Module):
    def __init__(self, c_out, resolution=[16, 16], heads=4, reduction=16) -> None:
        super().__init__()
        self.hydra_attention_1 = HydraAttention(int(c_out//2), reduction=reduction, heads=heads, height=int(resolution[0]), width=int(resolution[1]))
        self.hydra_attention_2 = HydraAttention((c_out//2), reduction=reduction, heads=heads, height=int(resolution[0]), width=int(resolution[1]))

    def forward(self, x):
        x_ce = self.hydra_attention_1(x[:, :x.size(1)//2, :, :])
        x_t = self.hydra_attention_2(x[:, x.size(1)//2:, :, :])
        x = torch.cat((x_ce, x_t), dim=1)
        return x
    ...
    def __init__(self, n_branches, n_groups, pretrain_ongroups=True, end_bot_g=False, group_conv_mhsa=False, group_conv_mhsa_2=False, x2g = False, x4g=False):
        ...
        if n_groups > 0:
            ...
            for item in convlist:
                ...
                weight = m.weight[:int(m.weight.size(0)), :int(m.weight.size(1) / n_groups), :, :]
                if end_bot_g and item[1] == "conv2":
                    setattr(model_ft[int(item[0])], item[1], HydraAttention_2G(int(512), int(512)))
                elif group_conv_mhsa and item[1] == "conv2":
                    setattr(model_ft[int(item[0])], item[1], Conv_HydraAttention_4G(int(512), int(512)))
                elif group_conv_mhsa_2 and item[1] == "conv2":
                    setattr(model_ft[int(item[0])], item[1], Conv_HydraAttention_2G(int(512), int(512)))
                else:
                    if item[1] == "downsample":
                        getattr(model_ft[int(item[0])], item[1])[0] = nn.Conv2d(int(m.weight.size(1)), int(m.weight.size(0)), kernel_size=1, stride=1, groups=n_groups, bias=False).apply(weights_init_kaiming)
                        if pretrain_ongroups:
